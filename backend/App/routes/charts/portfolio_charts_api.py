...
def get_portfolio_summary():
    try:
        ...
        goal_skill_map = defaultdict(list)
        for g in goals:
            for raw in (g.required_skills or []):
                goal_skill_map[_normalize(raw)].append(g.goal_name or '')
        ...
        learning_timeline = sorted(
            [
                {
                    'year': yr,
                    'courses': data['courses'],
                    'self_studies': data['self_studies'],
                    'educations': data['educations'],
                    'skills_gained': sorted(data['all_skills']),
        ...
        for g in goals:
            ...
            for raw in (g.required_skills or []):
                ...
                skills_needed.append({
                    ...
                    'target': tgt,
                    'gap': max(tgt - current_score, 0),
                })
            skills_needed.sort(key=lambda x: -x['gap'])

            goals_payload.append({
                'goal_name': g.goal_name or '',
                'sub_title': g.sub_title or '',
                'status': g.status or 'Planned',
        ...
        source_contribution_list = sorted(
            [
                {
                    'source': source_key,
